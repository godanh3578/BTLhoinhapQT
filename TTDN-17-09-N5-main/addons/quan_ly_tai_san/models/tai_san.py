...
class TaiSan(models.Model):
    ...
    expense_account_id = fields.Many2one('account.account', string='Tài khoản chi phí', required=True)
    depreciation_account_id = fields.Many2one('account.account', string='Tài khoản khấu hao', required=True)
    user_id = fields.Many2one('res.users', string='Người nhập', default=lambda self: self.env.user, required=True)
    approver_id = fields.Many2one('res.users', string='Người duyệt')
    payer_id = fields.Many2one('res.users', string='Người chi')
    approval_state = fields.Selection([
        ...
        ('paid', 'Đã chi'),
        ('rejected', 'Từ chối')
    ], string='Trạng thái phê duyệt', default='draft', required=True)
    ...
    def action_approve(self):
        ...
        # Tạo bút toán kế toán ghi tăng tài sản khi được duyệt
        move_vals = {
            'date': fields.Date.today(),
            'ref': f'Mua mới tài sản: {self.ten_tai_san}',
            'journal_id': self.env["account.journal"].search([('type','=','general')], limit=1).id,
            'line_ids': [
                (0, 0, {
                    'account_id': self.asset_account_id.id,
                    'name': f'Ghi tăng tài sản {self.ten_tai_san}',
                    'debit': self.gia_tri_ban_dau,
                    'credit': 0.0,
                }),
                (0, 0, {
                    'account_id': self.expense_account_id.id,
                    'name': f'Chi phí mua tài sản {self.ten_tai_san}',
                    'debit': 0.0,
                    'credit': self.gia_tri_ban_dau,
                }),
            ]
        }
    ...
    def action_pay(self):
        self.approval_state = 'paid'
        self.payer_id = self.env.user
    ...
    loai_tai_san = fields.Selection([
        ('chung', 'Tài sản chung'),
        ('rieng', 'Tài sản riêng')
    ], string='Loại tài sản (chung/riêng)', default='chung', required=True)
    the_chap = fields.Selection([
        ('khong', 'Không thế chấp'),
        ('co', 'Đang thế chấp')
    ], string='Trạng thái thế chấp', default='khong', required=True)
